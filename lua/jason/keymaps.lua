...
function M.setup(config)
  ...
  if keymaps.run then
    ...
    end, { desc = 'Run project', silent = true })
  end

  if keymaps.test then
    vim.keymap.set('n', keymaps.test, function()
      require('jason.executor').test()
    end, { desc = 'Run tests', silent = true })
  end

  if keymaps.clean then
    vim.keymap.set('n', keymaps.clean, function()
      require('jason.executor').clean()
    end, { desc = 'Clean build artifacts', silent = true })
  end

  -- ── Console (overseer-style task output + history) ────────────────────────
  -- Default: <leader>jo  (o for "output")
  local console_key = keymaps.console or '<leader>jo'
  vim.keymap.set('n', console_key, function()
    require('jason.console').toggle()
  end, { desc = 'Toggle Jason Task Console', silent = true })
end
