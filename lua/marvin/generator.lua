...
-- Well-known archetypes to show when ~/.m2 cache is empty (common on NixOS/fresh installs)
local BUILTIN_ARCHETYPES = {
	{
		group_id = "org.apache.maven.archetypes",
		artifact_id = "maven-archetype-quickstart",
		version = "1.5",
		label = "maven-archetype-quickstart",
		desc = "Simple Java project with JUnit",
	},
	{
		group_id = "org.apache.maven.archetypes",
		artifact_id = "maven-archetype-webapp",
		version = "1.5",
		label = "maven-archetype-webapp",
		desc = "Java web application (WAR)",
	},
	{
		group_id = "org.apache.maven.archetypes",
		artifact_id = "maven-archetype-simple",
		version = "1.5",
		label = "maven-archetype-simple",
		desc = "Minimal Maven project",
	},
	{
		group_id = "org.apache.maven.archetypes",
		artifact_id = "maven-archetype-j2ee-simple",
		version = "1.5",
		label = "maven-archetype-j2ee-simple",
		desc = "Simple J2EE application",
	},
}
		...
		on_exit = function()
			vim.schedule(function()
				if #archetypes == 0 then
					-- No local cache — offer built-in archetypes instead of giving up
					vim.notify(
						"[Marvin] No cached archetypes in "
							.. m2_repo
							.. "\nShowing built-in archetypes (Maven will download on first use).",
						vim.log.levels.INFO
					)
					M.show_archetype_menu(BUILTIN_ARCHETYPES)
				else
					-- Merge: built-ins first so they are always visible, then local cache
					local merged = {}
					local merged_seen = {}
...
function M.show_archetype_menu(archetypes)
	...
	ui().select(items, {
		...
		if choice then
			M.show_project_wizard(choice._archetype)
		end
	end)
end
		...
		on_exit = function(_, code)
			vim.schedule(function()
				if code == 0 then
					...
					-- Show the last few lines of output so the user can see what went wrong
					local tail = {}
					for i = math.max(1, #output - 15), #output do
						tail[#tail + 1] = output[i]
					end
					ui().notify(
