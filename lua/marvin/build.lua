...
-- Jason's multi-language build executor, living inside the marvin namespace.
-- Commands and keymaps reference  require('marvin.build')  instead of
-- the old  require('jason.executor').
...
function M.build_and_run(prompt_args)
  ...
  require('core.runner').execute_sequence(
    { { cmd = bc,                                             title = 'Build' },
      { cmd = rc .. (run_args ~= '' and ' ' .. run_args or ''), title = 'Run' } },
...
-- Backwards-compat shims (used by graalvm.lua internally)
function M.execute(cmd, cwd, title)
  require('core.runner').execute({
    cmd = cmd, cwd = cwd, title = title or cmd, term_cfg = tcfg(), plugin = 'jason',
  })
end
