...
local function sep(l)
	return { label = l, is_separator = true }
end
...
local EXTRAS = {
	...
	meson = function(p)
		local configured = vim.fn.isdirectory(p.root .. "/builddir") == 1 or vim.fn.isdirectory(p.root .. "/build") == 1
		return {
			sep("Meson"),
			item(
				"j_meson_setup",
			...
			item("j_meson_sync", "󰒓", "Sync meson.build", "Update sources, includes, and deps"),
		}
	end,
...
function M.show()
	...
	local prompt = is_single
			and string.format(
				"Jason  %s %s  %s  [%s]  — use Build System to create meson/cmake/make",
				icon,
				tool,
				pname,
				lang
	...
	ui().select(items, {
		...
		if choice then
			M._handle(choice.id, p, meta)
		end
	end)
end

-- ── Meson sync ─────────────────────────────────────────────────────────────────
function M.sync_meson_build(p)
	if not p then
		vim.notify("[Marvin] No project detected", vim.log.levels.WARN)
		return
	end
	local root = p.root
	local meson_path = root .. "/meson.build"
	if vim.fn.filereadable(meson_path) ~= 1 then
		vim.notify("[Marvin] meson.build not found in " .. root, vim.log.levels.WARN)
		return
	end

	-- Read existing meson.build to extract project name and version
	local f = io.open(meson_path, "r")
	if not f then
		vim.notify("[Marvin] Cannot read meson.build", vim.log.levels.ERROR)
		return
	end
	local content = f:read("*a")
	f:close()

	local project_name = vim.fn.fnamemodify(root, ":t") -- fallback to directory name
	local project_version = "0.1.0"
	local project_lang = "c" -- fallback

	-- Try to extract from project() line
	for line in content:gmatch("[^\r\n]+") do
		local name, lang, version = line:match("^project%(%s*'([^']+)'%s*,%s*'([^']+)'%s*,%s*version%p*%s*'([^']+)'")
		if name and lang and version then
			project_name = name
			project_lang = lang
			project_version = version
			break
		end
		-- Alternative format: project('name', 'lang', version : '1.0.0', ...)
		name, lang, version = line:match("^project%(%s*'([^']+)'%s*,%s*'([^']+)'%s*,%s*version%p*%s*'([^']+)'")
		if name and lang and version then
			project_name = name
			project_lang = lang
			project_version = version
		...
		-- Another alternative: project('name', 'lang', ['1.0.0'], ...)
		name, lang, version = line:match("^project%(%s*'([^']+)'%s*,%s*'([^']+)'%s*,%s*{([^}]+)}")
		if name and lang and version then
			-- extract version from the array
			local v = version:match("'([^']+)'")
			if v then
				project_name = name
				project_lang = lang
				project_version = v
	...
	-- Get the meson creator exposed functions
	local meson_creator = require("marvin.meson_creator")
	local exposed = meson_creator.exposed

	-- Determine language (use inferred or from existing)
	local lang = exposed.infer_lang(root)
	if not lang then
		lang = project_lang -- fallback to extracted
	end
	if not lang then
		lang = "c" -- final fallback
	end

	-- Run auto detection (similar to wizard)
	local detected = exposed.auto_detect(root, lang)

	-- Collect sources
	local src_decl = exposed.collect_sources(root, lang)

	-- Collect include directories
	local inc_decl, inc_dirs = exposed.collect_include_dirs(root)

	-- Detect multiple main files
	local main_files = exposed.detect_main_files(root, lang)
	local multi_exe = #main_files > 1 and main_files or nil

	-- For multi-exe, build shared_src
	local shared_src_decl = src_decl
	if multi_exe then
		local main_paths = {}
		for _, m in ipairs(main_files) do
			main_paths[m.path] = true
		end
		local shared = {}
		local skip_patterns = { "/builddir/", "/build/", "/.marvin-obj/", "/.git/" }
		...
		table.sort(shared)
		shared_src_decl = #shared > 0 and "files(\n" .. table.concat(shared, ",\n") .. "\n)" or "files()"
	end
	...
	local opts = {
		...
		lang = lang,
		std = "c11", -- TODO: extract from existing meson.build or detect
		sanitizer = "none",
		testing = false,
		test_framework = "none",
		install = false,
		...
		test_src_decl = "files()", -- TODO: we could collect test sources
		inc_decl = inc_decl,
		inc_dirs = inc_dirs,
		protocol_entries = {}, -- TODO: we could detect wayland protocols
		multi_exe = multi_exe,
	}

	-- Generate meson.build content
	local new_content = exposed.meson_template(opts)

	-- Write the new content
	local f_out = io.open(meson_path, "w")
	if not f_out then
		vim.notify("[Marvin] Cannot write meson.build", vim.log.levels.ERROR)
		return
	end
	f_out:write(new_content)
	f_out:close()
	vim.notify("[Marvin] meson.build synced successfully", vim.log.levels.INFO)
	-- Restart LSP to pick up new compile_commands.json
	vim.defer_fn(function()
		vim.cmd("lsp restart")
		vim.notify("[Marvin] LSP restarted", vim.log.levels.INFO)
	end, 500)
end
...
function M._build_items(p, meta, has)
	...
	if has.build_run then
		add(item("j_build_run", "󰑓", "Build & Run", "Compile then run"))
	end
	if has.build then
		add(item("j_build", "󰑕", "Build", "Compile"))
	end
	if has.run then
		add(item("j_run", "󰐊", "Run", "Run"))
	end
	if has.test then
		add(item("j_test", "󰙨", "Test", "Run tests"))
	end
	if has.clean then
		add(item("j_clean", "󰃢", "Clean", "Remove artifacts"))
	end
	...
	if has.build or has.run or has.test_filter then
		...
		if has.build then
			add(item("j_build_args", "󰒓", "Build (args…)", "Extra build flags"))
		end
		if has.run then
			add(item("j_run_args", "󰒓", "Run (args…)", "Runtime arguments"))
		end
		if has.test_filter then
			add(item("j_test_filter", "󰍉", "Test (filter…)", "Specific test name"))
		end
	end
	...
	if has.fmt or has.lint or has.package or has.install then
		add(sep("Extras"))
		if has.fmt then
			add(item("j_fmt", "󰉣", "Format", "Auto-format"))
		end
		if has.lint then
			add(item("j_lint", "󰅾", "Lint", "Run linter"))
		end
		if has.package then
			add(item("j_package", "󰏗", "Package", "Create distributable"))
		end
		if has.install then
			add(item("j_install", "󰇚", "Install", "Install to local registry"))
		end
	end
...
function M.show_build_system_menu(p)
	...
	local function exists_tag(flag)
		return flag and "  (exists)" or ""
	end
...
function M.show_compile_commands_menu(p)
	...
	if has_meson_file and has_meson_bin then
		add({
			...
			desc = "meson setup builddir — compile_commands.json generated automatically",
		})
	end
...
function M._do_package_lib(opts)
	...
	for _, src in ipairs(sources) do
		...
		steps[#steps + 1] = string.format("%s %s %s %s -c %s -o %s", cc, std_flag, cflags, inc_str, esc(src), esc(obj))
	end

	steps[#steps + 1] = string.format("ar rcs %s %s", esc(archive), table.concat(vim.tbl_map(esc, obj_files), " "))
	...
	if vim.fn.isdirectory(hdr_dir) == 1 then
		...
		vim.notify(
			"[Marvin] Header directory not found: " .. hdr_dir .. "\nLibrary will be built without headers.",
			vim.log.levels.WARN
		)
	end
...
function M._offer_register_lib(root, dest_root, name)
	ui().select({
		{
			id = "yes",
			label = '󰐕 Yes — register "' .. vim.fn.fnamemodify(dest_root, ":~:.") .. '" as library search path',
		},
		{ id = "no", label = "󰅖 No thanks" },
	}, { prompt = "Register for auto-discovery?", format_item = plain }, function(ch)
		if not ch or ch.id == "no" then
			return
		end
...
function M._handle(id, p, meta)
	...
	elseif id == "j_meson_setup" then
		local configured = vim.fn.isdirectory(root .. "/builddir") == 1 or vim.fn.isdirectory(root .. "/build") == 1
		local cmd = configured and "meson setup --reconfigure builddir" or "meson setup builddir"
			...
			if ok then
				...
				vim.defer_fn(function()
					...
					if vim.fn.filereadable(src) == 1 and vim.fn.filereadable(dst) == 0 then
						...
						vim.notify(
							"[Marvin] compile_commands.json symlinked from builddir.\nRun :LspRestart",
							vim.log.levels.INFO
						)
					end
				end, 800)
	...
	elseif id == "j_meson_sync" then
		M.sync_meson_build(p)
	...
	elseif id == "ccmd_cmake" then
		run("cmake -B build -S . -DCMAKE_EXPORT_COMPILE_COMMANDS=ON", "Generate compile_commands.json", function(ok)
			if not ok then
				return
			end
			vim.defer_fn(function()
	...
	elseif id == "ccmd_bear_make" then
		run("bear -- make", "bear + make", function(ok)
			if ok then
				vim.notify("[Jason] compile_commands.json written.\nRun :LspRestart", vim.log.levels.INFO)
			end
		end)
	...
	elseif id == "ccmd_clangd_file" then
		...
		local flags = {}
		for _, f in ipairs(inc_flags) do
			flags[#flags + 1] = f
		end
		...
		if vim.fn.filereadable(path) == 1 then
			...
			}, { prompt = ".clangd already exists", format_item = plain }, function(ch)
				if ch and ch.id == "overwrite" then
					write_clangd()
				end
			end)
		else
